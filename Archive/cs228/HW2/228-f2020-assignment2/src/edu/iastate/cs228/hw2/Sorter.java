...
public abstract class Sorter {
    ...
    public void sortWithStatistics(WordList toSort, Comparator<String> comp, int totalToSort) throws NullPointerException, IllegalArgumentException {
        ...
        // Temporary storage of original list for reference
        WordList guineaPig = toSort.clone();

        CountingComparator<String> ccomp = new CountingComparator<>(comp);

        // do the testing for stats!
        long t = 0;
        int sortedSoFar = 0;
         do {
            toSort = guineaPig.clone();
            long t1 = System.nanoTime();
            sort(toSort, ccomp);
            t += System.nanoTime() - t1;
            sortedSoFar += toSort.length();
        } while (sortedSoFar < totalToSort);

         // Storing stats
        totalWordsSorted += sortedSoFar;
        totalSortingTime += t;
        totalComparisons += ccomp.getCount();

        // Output sorted list
        try {
            FileWriter out = new FileWriter(this.getName()+".txt", false);
            out.write(toSort.toString());
            out.close();
        } catch (IOException e) {
            e.printStackTrace();
        }
