...
public class Graph {
     ...
     * @throws IndexOutOfBoundsException if either of {@code fromIdx} and {@code toIdx} are invalid vertex
     *                                   indices
    ...
    public void addEdge(int fromIdx, int toIdx, int index, String word)
        ...
        Edge edge1 = new GraphEdge(index, word, vertices[fromIdx], vertices[toIdx]);
        vertices[fromIdx].edges().add(edge1);

        Edge edge2 = new GraphEdge(index, word, vertices[toIdx], vertices[fromIdx]);
        vertices[toIdx].edges().add(edge2);
    }
    ...
    private class GraphVertex implements Vertex {
        ...
        public boolean hasCycle(Vertex from) {
            for (Edge e : edges) {
              // Vertex with Edge to self considered a cycle
                if (e.getTo() == from/* || e.getTo() == this*/) {
                    this.visit();
                    continue;
                }
                if (e.getTo().isVisited()) return true;
                else {
                    this.visit();
                    if (e.getTo().hasCycle(this)) return true;
                }
            }
